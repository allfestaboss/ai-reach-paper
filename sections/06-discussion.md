...
than relied upon as a by-product. Five changes follow, all cheap.
...
five that does not route through the benchmark at all. §3.1.8 reports a grader
that awarded full marks to a submission which had read nothing, in a benchmark
whose calibration and adversarial suite both passed and whose solvers had no
way to observe the fault. What located it was an outside reading of the method,
and what made the diagnosis possible was version control: the pre-revision
grader could be restored and the submissions re-scored, which is how the stated
reason for the withdrawal was itself shown to be wrong. Two habits follow, and
both are cheap. Commit the task statement, reference, grader and adversarial
suite before any arm runs, and do not touch them afterwards — a revision made
after reading the submissions moved 52.6 points, more than the effect being
reported. And put at least one uninformed submission in the adversarial suite:
a suite generated by perturbing the reference tests only the reference's
neighbourhood, and the cheapest probe of everywhere else is a submission that
read nothing.
...
remains uncontrolled and unmeasured. The claim is that three detectors
demonstrably worked here — the benchmark's own checks, the subject under test,
and an outside reading of the method — that their blind spots are complements
rather than duplicates, and that all three can be operated on purpose.
